...
const starterGuidance = "Tell us what is happening with the pool, equipment, or part. A short note is enough to start narrowing the path.";

function getFallbackGuidedResponse(question: string) {
  const text = question.toLowerCase();
  if (!question.trim()) return starterGuidance;
  if (text.includes("pump") || text.includes("humming") || text.includes("motor")) return "This sounds pump-related. A photo of the pump label, capacitor area, basket lid, or the failed part can help Big Tex narrow the replacement path quickly.";
  if (text.includes("pressure") || text.includes("flow") || text.includes("circulation")) return "Low pressure or weak circulation can point to filter, valve, basket, pump, or cleaner-side issues. A photo of the equipment pad and the part in question is the fastest next step.";
  ...
  if (text.includes("valve") || text.includes("fitting") || text.includes("seal") || text.includes("gasket")) return "That is exactly the kind of part request where a photo helps. Send the part, the connection points, and any visible numbers so Big Tex can source the right fit.";
  if (text.includes("commercial") || text.includes("route") || text.includes("property") || text.includes("hoa")) return "For route or property support, include the item needed, timing pressure, and whether pickup or delivery is preferred. Big Tex can help turn that into a fulfillment path.";
...
export default function HomePage() {
  ...
  const localGuidedResponse = useMemo(() => getFallbackGuidedResponse(question), [question]);
  ...
  const mailBody = encodeURIComponent(["Pool part help request", "", `Name: ${name || ""}`, `Reply to: ${replyTo || ""}`, `Photo selected: ${file?.name || "No photo selected"}`, `Question: ${question || ""}`, `Description: ${description || ""}`, "", "Please attach the part photo before sending if it did not attach automatically from the website."].join("\n"));
  ...
  async function handleGuideRequest() {
    ...
    try {
      const response = await fetch("/api/bigtex-intake", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "guide", question: trimmedQuestion }) });
    ...
    <main>
      ...
      <section id="contact" className="section contactSection">
        <div className="contact contactWithImage intakeContact">
          ...
          <div className="contactContent"><div className="eyebrow">Big Tex Part Finder</div><h2>Don’t know the part? Send it or ask.</h2><p>Upload a photo, describe the problem, or ask what you need. Big Tex will help identify the right product, replacement part, or supply path so the pool gets back online faster.</p><div className="contactChips" aria-label="Fast support options">{contactChips.map((chip) => (<span key={chip}>{chip}</span>))}</div><p className="responseNote">Typical response: 10–15 minutes during business hours.</p><p className="contactDetail">{contact.address}</p><p className="contactDetail">{contact.email}</p></div>
          <form className="intakePanel" onSubmit={handleSubmit}>
            ...
            {mode === "photo" ? (<label className="uploadBox"><input type="file" accept="image/*" capture="environment" onChange={handleFileChange} /><span className="uploadIcon">＋</span><strong>{file?.name || "Take a photo of the part or equipment"}</strong><small>Not sure what it is? Send what you see: part, label, equipment pad, valve, basket, seal, or fitting.</small></label>) : (<div className="askBox"><label htmlFor="part-question">What is happening?</label><textarea id="part-question" value={question} onChange={(event) => { setQuestion(event.target.value); setGuideStatus("idle"); }} placeholder="Example: Pump is humming but not starting. I need the right part today." rows={4} /><div className="quickPrompts" aria-label="Common questions">{quickPrompts.map((prompt) => (<button type="button" key={prompt} onClick={() => { setQuestion(prompt); setGuideStatus("idle"); }}>{prompt}</button>))}</div><button className="guideButton" type="button" onClick={handleGuideRequest} disabled={guideStatus === "loading"}>{guideStatus === "loading" ? "Checking..." : "Get guided answer"}</button><div className="solaceLite"><span>{guideStatus === "error" ? "Fallback guidance" : "Guided intake"}</span><p>{activeGuidance}</p></div></div>)}
            <label className="fieldLabel" htmlFor="part-description">Short note</label><textarea id="part-description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Brand, model, urgency, pickup or delivery, commercial route details..." rows={3} />
